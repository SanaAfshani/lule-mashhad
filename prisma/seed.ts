...
async function main() {
  ...
  // ─── Testimonials ─────────────────────────────────────
  const testimonials = [
    { name: 'مهندس طاهری', company: 'اداره آب و فاضلاب مشهد', position: 'مدیر بهره‌برداری', content: 'چندین سال است که لوله‌های پلی اتیلن و چدن داکتیل مورد نیاز شبکه را از لوله آنلاین مشهد تامین می‌کنیم. کیفیت و تحویل به موقع همیشه مطلوب بوده است.', rating: 5 },
